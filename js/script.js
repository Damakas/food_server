...
window.addEventListener('DOMContentLoaded', function () {
  ...
  // Функция для выозова fetch метод GET
  ...
  //Пример с использованием библиотеки axios
  ...
  // Способ который не будет использовать классы,использвуется в случае если нужно что=то построить 1 раз
  // getResource('http://localhost:3000/menu')
  ...
  // Функция для выозова fetch метод POST
...
`;
  ...
  next.addEventListener('click', () => {
    if (offset == digitSort(width) * (slides.length - 1)) {
      offset = 0
    } else {
      offset += digitSort(width)
    }
  ...
  prev.addEventListener('click', () => {
    if (offset == 0) {
      offset = digitSort(width) * (slides.length - 1)
    } else {
      offset -= digitSort(width)
    }
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', (e) => {
      ...
      slidesIndex = slideTo;
      offset = digitSort(width) * (slideTo - 1)
